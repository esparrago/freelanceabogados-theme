<div class="row single-posted-by-widget">
	...
	<div class="large-8 small-8 columns posted-by-widget">
		<p><span><?php echo __( 'Publicado por:', APP_TD ); ?></span>
			<span class="meta name"><?php echo html_link( get_the_hrb_user_profile_url( $user ), $user->display_name ); ?></span>
			<span class="meta member"><?php echo __( 'Miembro desde:', APP_TD ); ?> <?php the_hrb_user_member_since( $user ); ?></span>
			<span class="meta location"><i class="icon i-user-location"></i> <?php the_hrb_user_location( $user ); ?></span>
			<span class="meta rating"><?php echo __( 'Clasificación:', APP_TD ); ?> <?php the_hrb_user_avg_rating( $user ); ?> (<?php echo sprintf( _n( '1 Reseña', '%d Reseñas', $user_reviews, APP_TD ), $user_reviews ); ?>)</span>
		</p>
	</div>
</div>
...
<div class="single-by-widget-meta row">
	<div class="large-12 columns">
		...
		<div class="row more-projects-link">
			<div class="columns">
				<?php echo html_link( get_the_hrb_user_profile_url( $user ), sprintf( __( 'Ver todos los proyectos de %s', APP_TD ), $user->display_name ) ); ?>
			</div>
		</div>
	</div>
</div>