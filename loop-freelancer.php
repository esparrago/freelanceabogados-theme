	...
	<?php else: ?>
		...
		<article class="freelancer content-no-results">
			<?php if ( is_search() && get_query_var('s') ) : ?>
				...
				<h5 class="no-results"><?php printf( __( 'No se encontraron abogados con el nombre: "%s".', APP_TD ), hrb_get_search_query_var( 'ls' ) ); ?></h5>

			<?php else: ?>

				<h5 class="no-results"><?php echo __( 'No se encontraron abogados', APP_TD ); ?></h5>
